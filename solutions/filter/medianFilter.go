...
func filter(filepathIn, filepathOut string, threads int) {
	...
	if threads == 1 {
		...
		// Create a slice to hold channels
		var dataChannels []chan [][]uint8

		// Allocate channels in the slice
		numChannels := threads
		dataChannels = make([]chan [][]uint8, numChannels)

		// Initialise each channel
		for i := 0; i < numChannels; i++ {
			dataChannels[i] = make(chan [][]uint8)
		}

		// Start worker goroutines
		for i := 0; i < threads; i++ {
			go worker(i*height/threads, (i*height/threads)+height/threads, 0, height, immutableData, dataChannels[i])
		}
...
// Worker function that invokes func with passed args in parallel using threads.
